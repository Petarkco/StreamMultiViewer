...
function layoutGrid() {
	...
	const aspectW = 16,
		aspectH = 9;
	const WIDTH_WASTE_WEIGHT = 1.0;
	const HEIGHT_WASTE_WEIGHT = 1.45;
	const EPSILON = 1e-3;
	let best = null;
	for (let cols = 1; cols <= n; cols++) {
		...
		const weightedWaste =
			widthWaste * WIDTH_WASTE_WEIGHT + heightWaste * HEIGHT_WASTE_WEIGHT;
		...
		if (
			...
			candidate.heightWaste < best.heightWaste - EPSILON
		) {
			best = candidate;
			continue;
		}
		if (
			Math.abs(candidate.weightedWaste - best.weightedWaste) <= EPSILON &&
			Math.abs(candidate.heightWaste - best.heightWaste) <= EPSILON &&
		...
		if (
			Math.abs(candidate.weightedWaste - best.weightedWaste) <= EPSILON &&
			Math.abs(candidate.heightWaste - best.heightWaste) <= EPSILON &&
			Math.abs(candidate.widthWaste - best.widthWaste) <= EPSILON &&
